...
export class ExportService {
  ...
  static exportActionsAsCSV(meetingJson: any) {
    ...
    actionsData.forEach((action: any) => {
      const cell = (value: unknown) => {
        ...
        // Prevent meeting text from becoming a spreadsheet formula on export.
    ...
    this.downloadBlob(
      csv,
      `${title.replace(/\s+/g, "_")}_Actions.csv`,
      "text/csv",
    );
  }
  ...
  static exportSummaryAsMarkdown(meetingJson: any) {
    ...
    this.downloadBlob(
      md,
      `${title.replace(/\s+/g, "_")}_Summary.md`,
      "text/markdown",
    );
  }
  ...
  static exportExecutiveBriefAsMarkdown(meetingJson: any) {
    ...
    this.downloadBlob(
      md,
      `${title.replace(/\s+/g, "_")}_Executive_Brief.md`,
      "text/markdown",
    );
  }
  ...
  static exportEmailAsTXT(meetingJson: any) {
    ...
    this.downloadBlob(
      txt,
      `${title.replace(/\s+/g, "_")}_Email.txt`,
      "text/plain",
    );
  }
